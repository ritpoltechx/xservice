...
const STYLES = `
  ...
  @keyframes logo-pulse {
    0%, 100% { transform: scale(1);    filter: drop-shadow(0 0  8px rgba(139,92,246,.5)) }
    50%       { transform: scale(1.08); filter: drop-shadow(0 0 24px rgba(139,92,246,.9)) }
  }
...
function XLogo({ size = 72, animated = false }: { size?: number; animated?: boolean }) {
  return (
    <svg width={size} height={size} viewBox="0 0 52 52" fill="none" style={animated ? { animation: "logo-pulse .9s ease-in-out 1.1s 2" } : undefined}>
      <defs>
      ...
      {animated ? (
        /* Outlined version that "draws" in, then fills */
        <text
          x="4" y="46"
          ...
          strokeDasharray="400"
          style={{
            animation: "logo-draw .55s cubic-bezier(.4,0,.2,1) .2s both, logo-fill .35s ease .72s both",
          }}
        >X</text>
      ) : (
        <text
...
/* Total preloader duration in ms — must match CSS timing */
...
export default function OrganizationsPage() {
  ...
  useEffect(() => {
    /* Start exit animation ~400 ms before unmount */
      ...
      >
            ...
            >
              <div
                style={{
                  height: "100%",
                  borderRadius: 999,
                  background: "linear-gradient(90deg, #5E6AD2, #8B5CF6, #EC4899)",
        ...
        <div
          style={{
            ...
            flexDirection: "column",
            alignItems: "center",
            boxShadow: "0 32px 64px -16px rgba(0,0,0,.7), 0 0 0 1px rgba(255,255,255,.05)",
        ...
        >
          ...
          >
            ...
            >
              {ORGS.map((o) => (
                <option key={o.value} value={o.value} style={{ background: "#17181c" }}>
                  {o.label}
                </option>
              ))}
            </select>
            <svg
              style={{ position: "absolute", right: 12, top: "50%", transform: "translateY(-50%)", color: "var(--txt-3)", pointerEvents: "none" }}
          ...
          <button
            ...
            onMouseLeave={(e) => {
              e.currentTarget.style.background = "linear-gradient(180deg, rgba(255,255,255,.12) 0%, rgba(255,255,255,.06) 100%)"
